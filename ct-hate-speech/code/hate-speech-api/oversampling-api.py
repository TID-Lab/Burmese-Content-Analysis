...
# for minority_class_size in resampled_minority_class_sizes:
    ...
    # # with open("X.bin", "wb") as X_out:
        # # pickle.dump(X, X_out)
    # # with open("y.json", "wb") as y_out:
        # # pickle.dump(y, y_out)
    
    # with open("X.bin", "rb") as X_in:
        # X =pickle.load(X_in)
    # with open("y.bin", "rb") as y_in:
        # y= pickle.load(y_in)
    # y_counter = Counter(y)
...
# with open("api/clfs/oversampled-{}.pkl".format(clf_name), "wb") as clf_f:
